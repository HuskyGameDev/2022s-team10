...
public class Spikes : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        if(collision.gameObject.tag.Equals("Player")) { //if player collides with spikes
            ...
            //knockback
            collision.attachedRigidbody.velocity += new Vector2(Mathf.Sign(collision.transform.position.x - transform.position.x) * knockback, Mathf.Sign(collision.transform.position.x - transform.position.x) * knockback);
